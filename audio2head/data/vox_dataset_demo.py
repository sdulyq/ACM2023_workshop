...
class VoxDataset(Dataset):
    ...
    def __getitem__(self, idx): # 注意这里的idx很大, 是全部的append后的
        ...
        audio = torch.from_numpy(self.data[idx]['audio']) # 这里audio一直在变，但是speaker——video不变
        audio = self.audio_transform(audio)
        
       
        speaker_video = self.data[idx]['speaker']
        speaker_3dmm_fixed,  speaker_3dmm_dynam  = self.load_3dmm(speaker_video) # 这个是按idx取3dmm参数，这样音频和参数是一一对应的了
        ...
        return audio, init_signal, target_signal, speaker_id,name,speaker_3dmm_fixed
    ...
    def load_3dmm(self, data):
        ...
        id_gamma_tex    = torch.from_numpy(id_gamma_tex).float()
        angle_exp_trans = torch.from_numpy(angle_exp_trans).float()
